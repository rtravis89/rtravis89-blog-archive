...
player <- read_csv("C:\\Users\\ryan\\Documents\\NFL\\nfl_00-17\\PLAYER.csv")
game <- read_csv("C:\\Users\\ryan\\Documents\\NFL\\nfl_00-17\\GAME.csv")
injury <- read_csv("C:\\Users\\ryan\\Documents\\NFL\\nfl_00-17\\INJURY.csv")
offense <- read_csv("C:\\Users\\ryan\\Documents\\NFL\\nfl_00-17\\OFFENSE.csv")
...
For simplicity (i.e. I'm lazy) I'm instead going to devide the number of games missed by 16, which transforms the outcome into the proportion of games missed out of 16. This allows us to fit a zero-one inflated beta regression. This model is a mixture distribution like the zero inflated poisson, except there are now two binomial processes that account for the values 0 and 1. The beta distribution is then used as the model for all values inbetween. One big issue with this model is that it's continuously valued between the [0,1] interval, where as our data was originally integers. 
...
####Posterior Predictive Checks
```{r}
pp_check(fit1, nsamples = 20)
pp_check(fit1, nsamples = 20, type = "ecdf_overlay")
```

We can see from the plots above that the zero one inflated beta model fits the tails well, but does a poor job inbetween. Since about 70% of the data is either a 0 or a 16, the improved fit in the tails will likely improve the model performance overall. We can compare the two models using the LOO function, which performs approximate leave-one-out cross-validation.
...
Based on the leave-one-out cross validation the zero-one beta regression provides a better fit. Like I said above, this is likely due to the improved fit in the tails, but at the cost of worse fit in between. 
