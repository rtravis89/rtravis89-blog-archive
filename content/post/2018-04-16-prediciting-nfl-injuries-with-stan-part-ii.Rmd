...
The first model I'm going to fit is a zero inflated poisson with a varying intercept by player (to account for the correlation across years). 
$$Y \sim ZIPoisson(p_i, \lambda_i) $$
$$logit(z_i) \sim \alpha_z + \alpha_{player_z} + \beta_z*X $$
$$log(\lambda_i) \sim \alpha_\lambda + \alpha_{player_\lambda} + \beta_\lambda*X $$
```{r, message = FALSE, warning = FALSE, cache = TRUE}
fit0 <- brm(bf(games.inj ~ year + pos1 + height + weight + age + (1|player),
               zi ~ year + pos1 + height + weight + age + (1|player)),
